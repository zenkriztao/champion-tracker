...
/**
 Enumeration for the available endpoints at champion-tracker backend
 */
enum RESTRoutes: URLRequestConvertible {
  ...
  func asURLRequest() throws -> URLRequest {
    guard let url = URL.init(string: self.route.path, relativeTo: URL.init(string: "https://champion-tracker.net")) else {
      let error = NSError(domain: "AMPErrorDomain", code: -1)
      throw error
    }
